...
} = require("../validations/historySellAnimals");
...
class FarmController {
  ...
  static async InsertFarm(req, res) {
    try {
      ...
      const result = await FarmRepository.InsertFarm(
        farmName,
        createDate,
        status,
        animalType_ID,
  ...
  static async GetAllFarms(req, res) {
    ...
    if (result == null) {
      ...
      ReturnResponseUtil.returnResponse(
        ...
        200,
        true,
        "Get All Animal Types Successfully",
        result
      );
    }
  }
  ...
  static async UpdateFarmByID(req, res) {
    try {
      ...
      var id = req.params.id;
      var result = await FarmRepository.UpdateFarmByID(
        farmName,
        status,
        animalType_ID,
        animalDensity,
    ...
    try {
      ...
      for (let index = 0; index < list_users.length; index++) {
        if (
          ...
          // b2: thêm tài khoản
          const { insertId: userAccount_ID } =
            await UserDetailRepository.InsertUserAccount(
              phoneNumber,
              password,
              createDate,
              userDetail_ID
      ...
      ReturnResponseUtil.returnResponse(
        ...
        200,
        true,
        `Insert ${numberSuccess} users To Farm Successfully`,
        users
      );
    } catch (error) {
      // console.log(error);
  ...
  static async ReportAnimalSummary(req, res) {
    ...
    if (result != null) {
      ReturnResponseUtil.returnResponse(
        ...
        200,
        true,
        "Get Report Animal Sumary Successfully",
        result
      );
    } else {
      ReturnResponseUtil.returnResponse(
    ...
    try {
      ...
      if (resultHistoryCageEntry) {
        if (cages.length > 0) {
          for (let i = 0; i < cages.length; i++) {
            ...
            await HistoryCageEntryDetailRepository.InsertHistory(
              cage_id,
              animalsEntry,
              resultHistoryCageEntry.insertId
            );
        ...
        ReturnResponseUtil.returnResponse(
          ...
          200,
          true,
          "Inserted history entry cage successfully"
        );
      }
    } catch (error) {
  ...
  static async ReportEntryCage(req, res) {
    ...
    if (result) {
      ReturnResponseUtil.returnResponse(
        ...
        200,
        true,
        "Get report entry cage successfully",
        result
      );
    } else {
      ReturnResponseUtil.returnResponse(
  ...
  static async ClassificationStatisticsOfAnimals(req, res) {
    try {
      ...
      const resultStandardAnimals = await FarmRepository.GetStandardAnimals(
        weight,
        farm_id
      );
      const resultSubStandardAnimals =
        await FarmRepository.GetSubStandardAnimals(weight, farm_id);
      ...
      ReturnResponseUtil.returnResponse(
        ...
        200,
        true,
        "Get classification statistics of animals successfully",
        data
      );
    } catch (error) {
      ReturnResponseUtil.returnResponse(
  ...
  static async GetAnimalSummaryOfEachCage(req, res) {
    ...
    if (result != null) {
      ReturnResponseUtil.returnResponse(
        ...
        200,
        true,
        "Get Report Animal Sumary Of Each Cage Successfully",
        result
      );
    } else {
      ReturnResponseUtil.returnResponse(
  ...
  static async SellAnimals(req, res) {
    try {
      ...
      if (EventRepository.isEventActive(events)) {
        ...
        var resultInsertHistorySellAnimals =
          await HistorySellAnimalsRepository.InsertHistory(
            user_id,
            farm_id,
            sellAnimals,
            totalWeightAnimals,
        ...
        if (resultInsertHistorySellAnimals) {
          ...
          if (cages.length > 0) {
            for (let i = 0; i < cages.length; i++) {
              ...
              await HistorySellAnimalsDetailRepository.InsertHistory(
                cage_id,
                sellAnimalsInCage,
                totalWeightAnimalsInCage,
                resultInsertHistorySellAnimals.insertId
              );
            }
            ReturnResponseUtil.returnResponse(
              res,
              200,
              true,
              "Inserted history sell animals successfully"
            );
          }
        }
      }
  ...
  static async InsertBuyer(req, res) {
    try {
      ...
      var result = await BuyerRepository.InsertBuyer(
        name,
        phone,
        ward_id,
        farm_id,
  ...
  static async InsertSupplier(req, res) {
    try {
      ...
      var result = await SupplierRepository.InsertSupplier(
        farm_id,
        name,
        phone,
        ward_id,
      ...
      if (result) {
        ReturnResponseUtil.returnResponse(
          ...
          200,
          true,
          "Inserted supplier successfully"
        );
      }
    } catch (error) {
      ReturnResponseUtil.returnResponse(
  ...
  static async GetEmployeesInFarm(req, res) {
    ...
    var result = await UserDetailRepository.GetUsersByRoleInFarm(
      role_id,
      farm_id
    );
    if (result != null) {
      ReturnResponseUtil.returnResponse(
        ...
        200,
        true,
        "Get Employees in farm successfully",
        result
      );
    } else {
      ReturnResponseUtil.returnResponse(
