...
} = require("../validations/userAccountSchema");
...
class UserAccountController {
  ...
  static async InsertUserAccount(req, res) {
    try {
      ...
      var searchUserAccount =
        await UserAccountRepository.SearchUserAccountByUsername(username);
      if (searchUserAccount.length > 0) {
        ReturnResponseUtil.returnResponse(res, 400, false, "Username existed");
      } else {
        var result = await UserAccountRepository.InsertUserAccount(
          username,
          password,
          createDate,
          userDetail_ID,
  ...
  static async GetUserAccountByID(req, res) {
    ...
    if (result.length > 0) {
      ReturnResponseUtil.returnResponse(
        ...
        200,
        true,
        "Get User Account By ID Successfully"
      );
    } else {
      ReturnResponseUtil.returnResponse(
        res,
  ...
  static async UpdateUserAccountByID(req, res) {
    try {
      ...
      var result = await UserAccountRepository.UpdateUserAccountById(
        password,
        id
      );
      if (result) {
