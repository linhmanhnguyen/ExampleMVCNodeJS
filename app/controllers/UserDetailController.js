...
class UserDetailController {
  ...
  static async InsertUserDetail(req, res) {
    try {
      ...
      var CheckUserDetailExistsByCitizenIDOrPhone =
        await UserDetailRepository.CheckUserDetailExistsByCitizenIDOrPhone(
      ...
      if (CheckUserDetailExistsByCitizenIDOrPhone.length > 0) {
        ReturnResponseUtil.returnResponse(
          ...
          400,
          false,
          "Citizen Identification ID or Phone existed"
        );
      } else {
        var result = await UserDetailRepository.InsertUserDetail(
          fullname,
          dateOfBirth,
          gender,
          citizenIdentification_ID,
        ...
        if (result) {
          ReturnResponseUtil.returnResponse(
            ...
            200,
            true,
            "Created User Detail successfully",
            result.insertId
          );
        }
      }
  ...
  static async GetAllUserDetails(req, res) {
    ...
    if (result.length > 0) {
      ReturnResponseUtil.returnResponse(
        ...
        200,
        true,
        "Get All User Details Successfully",
        result
      );
    } else {
      ReturnResponseUtil.returnResponse(
  ...
  static async GetUserDetailsByID(req, res) {
    ...
    if (result.length > 0) {
      ReturnResponseUtil.returnResponse(
        ...
        200,
        true,
        "Get User Detail By ID Successfully",
        result
      );
    } else {
      ReturnResponseUtil.returnResponse(
  ...
  static async UpdateUserDetail(req, res) {
    try {
      ...
      var result = await UserDetailRepository.UpdateUserDetailByID(
        fullname,
        dateOfBirth,
        gender,
        citizenIdentification_ID,
