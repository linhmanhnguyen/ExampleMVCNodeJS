...
class HistoryAnimalTransferCageRepository {
  /**
  ...
  static async GetHistoryByAnimalID(Animal_ID) {
    const query =
      "SELECT * FROM history_animal_transfer_cage WHERE animal_id = ?";
  ...
  static async GetHistoryByID(History_ID, Animal_ID) {
    const query =
      "SELECT * FROM history_animal_transfer_cage WHERE id = ? AND animal_id = ?";
  ...
  static async DeleteHistoryByID(History_ID, Animal_ID) {
    const query =
      "DELETE FROM history_animal_transfer_cage WHERE id = ? AND animal_id = ?";
    const params = [History_ID, Animal_ID];
    const result = await connection.query(query, params);
    return result;
  }
  ...
  static async DeleteHistoriesOfAnimal(Animal_ID) {
    const query =
      "DELETE FROM history_animal_transfer_cage WHERE animal_id = ?";
    const params = [Animal_ID];
    const result = await connection.query(query, params);
    return result;
  }
...
module.exports = HistoryAnimalTransferCageRepository;
